			...
			*/
			...
			else
			{
				...
				foreach ($_SESSION['F'] as $F_raw) 
				{
					if($F_raw[0] != '')
					{
						echo  '<li><b>' . $F_raw[2] . '<a href="action.php?del_F=' . $F_raw[0] . '"onclick="return confirm(\'Êtes-vous sûr ?\')">&nbsp&nbspX</a></b></li>	<ul>';
						foreach ($_SESSION['SF'][$i] as $SF_raw) 
						{
							if($SF_raw[2] != '')
								echo  '<li>' . $SF_raw[2] . '<a href="action.php?del_SF=' . $SF_raw[0] . '" onclick="return confirm(\'Êtes-vous sûr ?\')">&nbsp&nbspX</a></li>';
						}
